...
class GeneticAlgorithm:
    ...
    def geneticAlgorithm(self):
        ...
        for i in range(self.maxGeneration):
            ...
            #parent1, parent2 = self.selectionTournament(2)  # Tournament selection

            parent1, parent2 = self.selectionRandom(2)  # Random selection of parents
    ...
    def selectionTournament(self, k):
        parents = []
        for i in range(k):
            tournamentSize = random.randint(1, len(self.population))
            tournament = self.selectionRandom(tournamentSize)
            aspirants = list(map(lambda p: self.generateFitness(p), tournament))
            maxIndex = aspirants.index(max(aspirants))
            parents.append(tournament[maxIndex])
        return parents

    '''Random selection function'''
    def selectionRandom(self, k):
        return [random.choice(self.population) for i in range(k)]
